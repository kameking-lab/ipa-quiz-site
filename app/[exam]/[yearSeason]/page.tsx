...
function parseYearSeason(slug: string): { year: number; season: Season } | null {
  const m = /^(\d{4})-(spring|autumn|cbt|published|first|second|early|may|september|january|october|annual)$/.exec(slug);
  if (!m) return null;
        ...
        <header className="mb-8 animate-fade-in">
          ...
          {code === "kaigo" && (
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              第38回（試験日 令和8年1月25日）の全125問です。問題文・選択肢は公式問題PDFの原文のまま掲載しています。解説は過去問AIが独自に作成したもので、公益財団法人社会福祉振興・試験センターとは関係ありません。過去問題には、その後の法改正等により現時点では問題として成立していないものが含まれる場合があります。
            </p>
          )}

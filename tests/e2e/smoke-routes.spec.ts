...
// 恒久削除ルートは middleware の GONE_PATHS で 410 Gone を返す（クロール資産回復の
// ため 404 より強いシグナル）。/pricing・/commerce は 410。/tokutei・/checkout は
// 後継も痕跡も無い純・意図的 404。両者を分けて検証する。
const GONE_410_ROUTES = ["/pricing", "/commerce"] as const;
const INTENTIONAL_404_ROUTES = ["/tokutei", "/checkout"] as const;
...
test.describe("smoke: 45 routes", () => {
  ...
  test("恒久削除ルートは 410 Gone、純・意図的ルートは 404", async ({ request }) => {
    expect(GONE_410_ROUTES.length).toBe(2);
    expect(INTENTIONAL_404_ROUTES.length).toBe(2);
    for (const path of GONE_410_ROUTES) {
      const res = await request.get(path);
      expect(res.status(), `${path} が 410 を返さない`).toBe(410);
    }
  ...
  test("ルート総数 = 45（必須200×15 + 試験区分13 + 書籍13 + 410Gone×2 + 意図的404×2）", () => {
    const total =
      REQUIRED_200_ROUTES.length +
      EXAM_ROUTES.length +
      BOOK_ROUTES.length +
      GONE_410_ROUTES.length +

...
export function SafetySiteHeader() {
  ...
  return <header className="sticky top-0 z-40 border-b border-border bg-background/95">
    <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4 sm:px-6">
      ...
      <nav aria-label="グローバルナビゲーション" className="hidden items-center gap-2 md:flex">
        {SAFETY_NAV.map(item => <Link key={item.href} href={item.href} aria-current={pathname === item.href ? "page" : undefined} className="rounded-lg px-3 py-3 text-sm font-medium hover:bg-muted">{item.label}</Link>)}
      </nav>
      <Sheet open={open} onOpenChange={setOpen}><SheetTrigger asChild><button aria-label="メニューを開く" className="flex h-11 w-11 items-center justify-center md:hidden"><Menu className="h-5 w-5" /></button></SheetTrigger>
        <SheetContent><SheetHeader><SheetTitle>安全衛生の学習</SheetTitle></SheetHeader>
          <nav aria-label="モバイルナビゲーション" className="mt-5 flex flex-col">
            {SAFETY_NAV.map(item => <SheetClose asChild key={item.href}><Link href={item.href} className="rounded-lg px-3 py-4 text-sm hover:bg-muted">{item.label}</Link></SheetClose>)}
          </nav>
        </SheetContent>
      </Sheet>
    </div>

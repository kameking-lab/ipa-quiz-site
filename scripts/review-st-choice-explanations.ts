...
interface ReceiptLedger {
  version: 1;
  scope: { exam: "st"; years: number[]; totalQuestions: number; totalChoices: number };
...
function importLegacyReceipts(
  ...
  for (const acceptedName of files) {
    ...
    const stIds = Object.keys(accepted).filter((id) => questions.has(id));
...
function makePrompt(batch: Question[]): string {
  ...
  return `あなたはIPA ITストラテジスト試験の独立査読者です。候補の全肢解説を公式問題・公式正答と技術一次資料に照らして査読してください。
...
async function main(): Promise<void> {
  const targets = ST_QUESTIONS.filter((question) => YEARS.has(question.year) && question.type === "multiple-choice" && question.choices);
  const groups = new Map<string, Question[]>();
  for (const question of targets) groups.set(paper(question), [...(groups.get(paper(question)) ?? []), question]);
  const expected = new Map([
    ["2024/spring/am1", 30], ["2024/spring/am2", 25],
    ["2025/spring/am1", 30], ["2025/spring/am2", 25],
  ]);
  if (targets.length !== 110 || groups.size !== expected.size) throw new Error(`unexpected ST scope: ${targets.length}/${groups.size}`);
  ...
  const ledger = readJson<ReceiptLedger>(RECEIPT_PATH, {
    version: 1,
    scope: { exam: "st", years: [2024, 2025], totalQuestions: 110, totalChoices: 440 },

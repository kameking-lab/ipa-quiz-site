...
SPECS = ROOT / "scripts/denken3-2025-figure-crops.json"
...
DIRECT = ROOT / "docs/evidence/denken3/direct"
...
def current_review(folder: Path, number: int, draft_hash: str, figure_hashes: dict[str, str],
                   row_hashes: dict[str, str], paper: dict, answer: dict) -> tuple[Path, dict, dict]:
    candidates = []
    for path in folder.glob("q*-direct-review.json"):
    ...
    if assessment["status"] != "PASS" or any(assessment.get(key) != [] for key in ISSUES):
        raise ValueError(f"Latest independent review is not PASS: {path} q{number}")
    if receipt["candidateSha256"][str(number)] != draft_hash:
        raise ValueError(f"Draft changed after independent review: {folder} q{number}")
    if receipt["officialRowSha256"][str(number)] != row_hashes:
        raise ValueError(f"Official row changed after independent review: {folder} q{number}")
    if receipt["sourceQuestionPdfSha256"] != paper["sha256"] or receipt["sourceAnswerPdfSha256"] != answer["sha256"]:
        raise ValueError(f"Official PDF changed after independent review: {folder} q{number}")
    ...
    return path, assessment, receipt
...
def promote(date: str, subject: str, numbers: list[int]) -> None:
    ...
    for number in numbers:
        ...
        row_hashes = {path: sha(ROOT / path) for path in page_map["questions"][str(number)]["images"]}
        receipt_path, assessment, receipt = current_review(folder, number, sha(draft_path), figure_hashes,
                                                            row_hashes, paper, session["officialAnswer"])
        ...
        for candidate in draft["units"]:
            ...
            rows.append(row)
        source_pack = {"questionPdfUrl": paper["url"], "questionPdfSha256": paper["sha256"],
                       "answerPdfUrl": session["officialAnswer"]["url"],
                       "answerPdfSha256": session["officialAnswer"]["sha256"],
                       "officialRowSha256": row_hashes, "figureSha256": figure_hashes,
                       "officialReferenceUrls": sorted({url for unit in draft["units"] for url in unit.get("officialReferenceUrls", [])})}
        DIRECT.mkdir(parents=True, exist_ok=True)
        direct_path = DIRECT / f"{date}-{subject}-q{number:02}.json"
        if direct_path.exists() and sha(direct_path) != sha(receipt_path):
            raise ValueError(f"Published direct receipt changed: {direct_path}")
        shutil.copyfile(receipt_path, direct_path)
        proofs.append({"number": number, "candidateSha256": sha(draft_path),
                       "candidateGroupSha256": receipt["candidateGroupSha256"],
                       "directReceipt": str(direct_path.relative_to(ROOT)).replace("\\", "/"),
                       "directReceiptSha256": sha(receipt_path), "assessment": assessment,
                       "officialAnswerImageSha256": receipt["officialAnswerImageSha256"],
                       "sourcePack": source_pack, "sourcePackSha256": canonical_sha(source_pack)})
    ...
    evidence.write_text(json.dumps({"status": "partial-accepted", "reviewMode": "direct-source-aware",

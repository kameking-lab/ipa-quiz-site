...
describe("2025年10月2級土木施工管理・照合済みNo.59〜66", () => {
  ...
  const review = JSON.parse(readFileSync(path.join(evidenceDir, "q59-66-opus-review-final-raw.json"), "utf8")) as typeof finalReview;
  const batchReceipt = JSON.parse(readFileSync(path.join(evidenceDir, "q59-66-release-receipt.json"), "utf8")) as typeof receipt & {
    figure: { publicPath: string; sha256: string; sourcePdfPage: number };
  };

  it("maps eight official questions, answer keys, 32 choice reasons and network figure", () => {
    expect(batch.questions.map((question) => question.number)).toEqual([59, 60, 61, 62, 63, 64, 65, 66]);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual([4, 4, 3, 2, 1, 2, 3, 3]);
    expect(batch.questions.map((question) => question.pdfPage)).toEqual([22, 22, 23, 23, 24, 24, 25, 25]);
    ...
    expect(batchReceipt.figuresRequired).toBe(1);
    ...
    for (const [index, record] of batch.questions.entries()) {
      ...
      expect(question.imageUrls?.[0]).toBe(record.imageUrl);
    ...
    expect(batchReceipt.figure.sourcePdfPage).toBe(23);
    expect(createHash("sha256").update(readFileSync(path.join(process.cwd(), "public", batchReceipt.figure.publicPath))).digest("hex")).toBe(batchReceipt.figure.sha256);
  });

  it("pins source bytes, candidate hash and actual first-party Opus 5.5 PASS", () => {
    ...
    expect(result.reviews.map((item) => item.number)).toEqual([59, 60, 61, 62, 63, 64, 65, 66]);
    expect(result.reviews.every((item) => item.verdict === "PASS" && item.issues.length === 0)).toBe(true);
  });
});

describe("2025年10月2級土木施工管理・照合済みNo.52〜58", () => {
  const batch = JSON.parse(readFileSync(path.join(process.cwd(), "data/questions/civil2/2025-october-batch-52-58.json"), "utf8")) as Omit<typeof candidate, "questions"> & {
    questionUrl: string;
    answerUrl: string;
    questions: (typeof candidate.questions[number] & { choices: string[]; choiceExplanations: string[] })[];
  };
  const review = JSON.parse(readFileSync(path.join(evidenceDir, "q52-58-opus-review-final-raw.json"), "utf8")) as typeof finalReview;
  const batchReceipt = JSON.parse(readFileSync(path.join(evidenceDir, "q52-58-release-receipt.json"), "utf8")) as typeof receipt;

  it("maps seven official questions and all 28 choice reasons to routes", () => {
    expect(batch.questions.map((question) => question.number)).toEqual([52, 53, 54, 55, 56, 57, 58]);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual([3, 4, 3, 3, 3, 4, 1]);
    expect(batch.questions.map((question) => question.pdfPage)).toEqual([20, 20, 20, 21, 21, 21, 21]);
    expect(batch.questions.map((question) => question.number)).toEqual(batchReceipt.questionNumbers);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual(batchReceipt.officialAnswerNumbers);
  ...
  it("pins source bytes, candidate hash and actual first-party Opus 5.5 PASS", () => {
    ...
    expect(result.overallVerdict).toBe("PASS");
    expect(result.reviews.map((item) => item.number)).toEqual([52, 53, 54, 55, 56, 57, 58]);
    expect(result.reviews.every((item) => item.verdict === "PASS" && item.issues.length === 0)).toBe(true);
  });
});

describe("2025年10月2級土木施工管理・照合済みNo.50〜51", () => {
  const batch = JSON.parse(readFileSync(path.join(process.cwd(), "data/questions/civil2/2025-october-batch-50-51.json"), "utf8")) as Omit<typeof candidate, "questions"> & {
    questionUrl: string;
    answerUrl: string;
    questions: (typeof candidate.questions[number] & { choices: string[]; choiceExplanations: string[]; imageUrl?: string })[];
  };
  const review = JSON.parse(readFileSync(path.join(evidenceDir, "q50-51-opus-review-final-raw.json"), "utf8")) as typeof finalReview;
  const batchReceipt = JSON.parse(readFileSync(path.join(evidenceDir, "q50-51-release-receipt.json"), "utf8")) as typeof receipt & {
    figure: { publicPath: string; sha256: string; sourcePdfPage: number };
  };

  it("maps both official questions, answer keys and the complete figure", () => {
    expect(batch.questions.map((question) => question.number)).toEqual([50, 51]);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual([1, 2]);
    expect(batch.questions.map((question) => question.pdfPage)).toEqual([19, 19]);
    expect(batch.questions.map((question) => question.number)).toEqual(batchReceipt.questionNumbers);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual(batchReceipt.officialAnswerNumbers);
    expect(batchReceipt.figuresRequired).toBe(1);
    expect(batch.questionUrl).toBe(sourceMap.questionUrl);
    ...
    expect(batchReceipt.figure.sourcePdfPage).toBe(19);
    expect(createHash("sha256").update(readFileSync(path.join(process.cwd(), "public", batchReceipt.figure.publicPath))).digest("hex")).toBe(batchReceipt.figure.sha256);
  });
  ...
  it("pins source bytes, candidate hash and actual first-party Opus 5.5 PASS", () => {
    ...
    expect(result.overallVerdict).toBe("PASS");
    expect(result.reviews.map((item) => item.number)).toEqual([50, 51]);
    expect(result.reviews.every((item) => item.verdict === "PASS" && item.issues.length === 0)).toBe(true);
  });
});

describe("2025年10月2級土木施工管理・照合済みNo.48〜49", () => {
  const batch = JSON.parse(readFileSync(path.join(process.cwd(), "data/questions/civil2/2025-october-batch-48-49.json"), "utf8")) as Omit<typeof candidate, "questions"> & {
    questionUrl: string;
    answerUrl: string;
    questions: (typeof candidate.questions[number] & { choices: string[]; choiceExplanations: string[]; imageUrl?: string })[];
  };
  const review = JSON.parse(readFileSync(path.join(evidenceDir, "q48-49-opus-review-final-raw.json"), "utf8")) as typeof finalReview;
  const batchReceipt = JSON.parse(readFileSync(path.join(evidenceDir, "q48-49-release-receipt.json"), "utf8")) as typeof receipt & {
    figure: { publicPath: string; sha256: string; sourcePdfPage: number };
  };

  it("maps both official questions, answer keys and the complete figure", () => {
    expect(batch.questions.map((question) => question.number)).toEqual([48, 49]);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual([1, 4]);
    expect(batch.questions.map((question) => question.pdfPage)).toEqual([18, 18]);
    expect(batch.questions.map((question) => question.number)).toEqual(batchReceipt.questionNumbers);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual(batchReceipt.officialAnswerNumbers);
    expect(batchReceipt.figuresRequired).toBe(1);
    expect(batch.questionUrl).toBe(sourceMap.questionUrl);
    ...
    expect(batchReceipt.figure.sourcePdfPage).toBe(18);
    expect(createHash("sha256").update(readFileSync(path.join(process.cwd(), "public", batchReceipt.figure.publicPath))).digest("hex")).toBe(batchReceipt.figure.sha256);
  });
  ...
  it("pins source bytes, candidate hash and actual first-party Opus 5.5 PASS", () => {
    ...
    expect(result.overallVerdict).toBe("PASS");
    expect(result.reviews.map((item) => item.number)).toEqual([48, 49]);
    expect(result.reviews.every((item) => item.verdict === "PASS" && item.issues.length === 0)).toBe(true);
  });
});

describe("2025年10月2級土木施工管理・照合済みNo.44〜47", () => {
  const batch = JSON.parse(readFileSync(path.join(process.cwd(), "data/questions/civil2/2025-october-batch-44-47.json"), "utf8")) as Omit<typeof candidate, "questions"> & {
    questionUrl: string;
    answerUrl: string;
    questions: (typeof candidate.questions[number] & { choices: string[]; choiceExplanations: string[] })[];
  };
  const review = JSON.parse(readFileSync(path.join(evidenceDir, "q44-47-opus-review-raw.json"), "utf8")) as typeof finalReview;
  const batchReceipt = JSON.parse(readFileSync(path.join(evidenceDir, "q44-47-release-receipt.json"), "utf8")) as typeof receipt;

  it("maps all four questions to official page, keys and routed explanations", () => {
    expect(batch.questions.map((question) => question.number)).toEqual([44, 45, 46, 47]);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual([3, 4, 1, 2]);
    expect(batch.questions.map((question) => question.pdfPage)).toEqual([17, 17, 17, 17]);
    expect(batch.questions.map((question) => question.number)).toEqual(batchReceipt.questionNumbers);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual(batchReceipt.officialAnswerNumbers);
    expect(batchReceipt.figuresRequired).toBe(0);
    expect(batch.questionUrl).toBe(sourceMap.questionUrl);
  ...
  it("pins source bytes, candidate hash and actual first-party Opus 5.5 PASS", () => {
    ...
    expect(result.overallVerdict).toBe("PASS");
    expect(result.reviews.map((item) => item.number)).toEqual([44, 45, 46, 47]);
    expect(result.reviews.every((item) => item.verdict === "PASS" && item.issues.length === 0)).toBe(true);
  });
});

describe("2025年10月2級土木施工管理・照合済みNo.40〜43", () => {
  const batch = JSON.parse(readFileSync(path.join(process.cwd(), "data/questions/civil2/2025-october-batch-40-43.json"), "utf8")) as Omit<typeof candidate, "questions"> & {
    questionUrl: string;
    answerUrl: string;
    questions: (typeof candidate.questions[number] & { choices: string[]; choiceExplanations: string[] })[];
  };
  const review = JSON.parse(readFileSync(path.join(evidenceDir, "q40-43-opus-review-raw.json"), "utf8")) as typeof finalReview;
  const batchReceipt = JSON.parse(readFileSync(path.join(evidenceDir, "q40-43-release-receipt.json"), "utf8")) as typeof receipt;

  it("maps all four questions to the official page, answers and routed explanations", () => {
    expect(batch.questions.map((question) => question.number)).toEqual([40, 41, 42, 43]);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual([1, 3, 4, 4]);
    expect(batch.questions.map((question) => question.pdfPage)).toEqual([16, 16, 16, 16]);
    expect(batch.questions.map((question) => question.number)).toEqual(batchReceipt.questionNumbers);
    expect(batch.questions.map((question) => question.officialAnswerNumber)).toEqual(batchReceipt.officialAnswerNumbers);
    expect(batchReceipt.figuresRequired).toBe(0);
    expect(batch.questionUrl).toBe(sourceMap.questionUrl);
  ...
  it("pins source bytes, candidate hash and actual first-party Opus 5.5 PASS", () => {
    ...
    expect(result.overallVerdict).toBe("PASS");
    expect(result.reviews.map((item) => item.number)).toEqual([40, 41, 42, 43]);
    expect(result.reviews.every((item) => item.verdict === "PASS" && item.issues.length === 0)).toBe(true);
  });
});

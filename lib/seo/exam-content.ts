...
export const EXAM_DEEP_CONTENT = {
  ...
  ap: {
    leadParagraph:
      "応用情報技術者試験（AP）は、IT エンジニアとして数年の実務経験を持つ中堅層の知識・応用力を測る国家試験です。従来の過去問は午前80問（多肢選択）と午後（記述）の構成です。2026年度はCBT方式の科目A・科目Bとして実施され、テクノロジから経営戦略・プロジェクト管理まで幅広い分野を横断的に問います。合格すると一部の高度試験で午前 I が 2 年間免除され、高度試験挑戦への現実的な足がかりとなる位置づけです。",
    mainTopics: [
  ...
  sc: {
    leadParagraph:
      "情報処理安全確保支援士試験（SC）は、サイバーセキュリティ分野唯一の登録制国家資格である情報処理安全確保支援士（登録セキスペ、英語名 RISS）の取得を目指す高度試験です。従来の過去問では午前 I・II と午後の記述・長文問題を通じて、脅威・脆弱性管理から実践的なセキュリティ設計・運用、関連法規までを問います。2026年度はCBT方式の前期・後期試験として実施されます。試験合格と支援士登録は別で、登録後の講習・更新条件はIPA公式案内を確認してください。",
    mainTopics: [
  ...
  nw: {
    leadParagraph:
      "ネットワークスペシャリスト試験（NW）は、ネットワーク分野の専門技術者として高度な設計・構築・運用能力を持つことを認定する高度試験です。従来の過去問では午前 I・II と午後 I・II の四段構成で、TCP/IP の挙動・ルーティング・冗長・無線・セキュリティなど幅広いプロトコルと、要件に基づく方式設計の判断力を問います。2025年度までは春期に実施されていました。2026年度はCBT方式の前期試験となるため、受験期間はIPA公式案内を確認してください。ベンダー資格と並んでネットワーク設計者のキャリア証明として高く評価されます。",
    mainTopics: [
  ...
  db: {
    leadParagraph:
      "データベーススペシャリスト試験（DB）は、データベースの設計・構築・運用・保守を担う専門技術者の能力を認定する高度試験です。従来の過去問では午前 I・II と午後 I・II の四段構成で、概念・論理・物理設計、SQL 記述、性能チューニング、トランザクション管理を実務に近い長文記述で問います。2025年度までは秋期に実施されていました。2026年度はCBT方式の後期試験となるため、受験期間はIPA公式案内を確認してください。データ基盤エンジニア・データアーキテクトの代表的なキャリア証明となります。",
    mainTopics: [
  ...
  st: {
    leadParagraph:
      "IT ストラテジスト試験（ST）は、経営戦略に基づき情報技術戦略を策定・実行する上位プロフェッショナル（CIO・DX 推進リーダー・IT コンサル）を認定する最高峰の高度試験です。従来の過去問では午前 I・II と午後 I（記述）・午後 II（論述・設問ごとに字数指定）の四段構成で、ビジネスモデル変革・IT 投資判断・全社業務改革の構想力を問います。2025年度までは春期に実施されていました。2026年度はCBT方式の前期試験となるため、受験期間はIPA公式案内を確認してください。難関論文試験の代表格として位置づけられます。",
    mainTopics: [
  ...
  sa: {
    leadParagraph:
      "システムアーキテクト試験（SA）は、情報システム全体のアーキテクチャ設計を統括する高度試験です。従来の過去問では午前 I・II と午後 I（記述）・午後 II（論文）の四段構成で、要件定義・方式設計・移行計画・非機能要件の判断力を、業務事例の論述を通じて問います。2025年度までは春期に実施されていました。2026年度はCBT方式の前期試験となるため、受験期間はIPA公式案内を確認してください。大規模システムの設計判断を担うリードアーキテクト・テックリードの代表的なキャリア証明として認知されています。",
    mainTopics: [
  ...
  pm: {
    leadParagraph:
      "プロジェクトマネージャ試験（PM）は、情報システム開発プロジェクトを統括する PM の能力を認定する高度試験です。従来の過去問では午前 I・II と午後 I（記述）・午後 II（論文）の四段構成で、PMBOK・PRINCE2 系の知識体系に加え、スコープ・コスト・スケジュール・品質・リスク・調達・ステークホルダーの統合管理力を業務事例論述で問います。2025年度までは秋期に実施されていました。2026年度はCBT方式の後期試験となるため、受験期間はIPA公式案内を確認してください。IT 業界では PMO・部長候補の到達点としても扱われます。",
    mainTopics: [
  ...
  es: {
    leadParagraph:
      "エンベデッドシステムスペシャリスト試験（ES）は、組込みシステム・IoT デバイスの専門技術者を認定する高度試験です。従来の過去問では午前 I・II と午後 I・II の四段構成で、RTOS・ハードウェア制御・タスクスケジューリング・組込み通信・機能安全といった、ハードウェアとソフトウェアの境界を扱う設計力を問います。2025年度までは秋期に実施されていました。2026年度はCBT方式の後期試験となるため、受験期間はIPA公式案内を確認してください。製造業の組込みエンジニア・IoT 設計者の代表的なキャリア証明として位置づけられています。",
    mainTopics: [
  ...
  sm: {
    leadParagraph:
      "IT サービスマネージャ試験（SM）は、IT サービスの安定運用とマネジメントを担うプロフェッショナルを認定する高度試験です。従来の過去問では午前 I・II と午後 I（記述）・午後 II（論文）の四段構成で、ITIL/ISO 20000 系のサービスマネジメント体系に基づくインシデント・問題・変更・構成・キャパシティ・継続性の各管理を、業務事例論述で問います。2025年度までは春期に実施されていました。2026年度はCBT方式の前期試験となるため、受験期間はIPA公式案内を確認してください。運用部門のリーダー・SRE マネージャの代表的キャリア証明です。",
    mainTopics: [
  ...
  au: {
    leadParagraph:
      "システム監査技術者試験（AU）は、情報システムの信頼性・安全性・効率性を独立した立場から監査する監査人の能力を認定する高度試験です。従来の過去問では午前 I・II と午後 I（記述）・午後 II（論文）の四段構成で、システム監査基準・管理基準・J-SOX・COBIT などを踏まえたリスクベースド監査の計画・実施・報告の判断力を、業務事例論述で問います。2025年度までは秋期に実施されていました。2026年度はCBT方式の後期試験となるため、受験期間はIPA公式案内を確認してください。内部監査人・IT コンサルの最終ゴールとして位置づけられます。",
    mainTopics: [

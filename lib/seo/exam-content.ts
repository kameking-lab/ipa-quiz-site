...
export const EXAM_DEEP_CONTENT: Record<ExamCode, ExamDeepContent> = {
  ...
  sc: {
    leadParagraph:
      "情報処理安全確保支援士試験（SC）は、サイバーセキュリティ分野唯一の登録制国家資格 RISS（登録情報セキュリティスペシャリスト）の取得を目指す高度試験です。午前 I・II と午後の記述・長文問題を通じて、脅威・脆弱性管理から実践的なセキュリティ設計・運用、関連法規までを問います。年 2 回（春・秋）実施で、登録後は 3 年ごとの講習で資格を維持します。",
    mainTopics: [
  ...
  nw: {
    leadParagraph:
      "ネットワークスペシャリスト試験（NW）は、ネットワーク分野の専門技術者として高度な設計・構築・運用能力を持つことを認定する高度試験です。午前 I・II と午後 I・II の四段構成で、TCP/IP の挙動・ルーティング・冗長・無線・セキュリティなど幅広いプロトコルと、要件に基づく方式設計の判断力を問います。秋期年 1 回実施で、ベンダー資格と並んでネットワーク設計者のキャリア証明として高く評価されます。",
    mainTopics: [
  ...
  es: {
    leadParagraph:
      "エンベデッドシステムスペシャリスト試験（ES）は、組込みシステム・IoT デバイスの専門技術者を認定する高度試験です。午前 I・II と午後 I・II の四段構成で、RTOS・ハードウェア制御・タスクスケジューリング・組込み通信・機能安全といった、ハードウェアとソフトウェアの境界を扱う設計力を問います。春期年 1 回実施で、製造業の組込みエンジニア・IoT 設計者の代表的なキャリア証明として位置づけられています。",
    mainTopics: [
  ...
  sm: {
    leadParagraph:
      "IT サービスマネージャ試験（SM）は、IT サービスの安定運用とマネジメントを担うプロフェッショナルを認定する高度試験です。午前 I・II と午後 I（記述）・午後 II（論文）の四段構成で、ITIL/ISO 20000 系のサービスマネジメント体系に基づくインシデント・問題・変更・構成・キャパシティ・継続性の各管理を、業務事例論述で問います。秋期年 1 回実施で、運用部門のリーダー・SRE マネージャの代表的キャリア証明です。",
    mainTopics: [
  ...
  au: {
    leadParagraph:
      "システム監査技術者試験（AU）は、情報システムの信頼性・安全性・効率性を独立した立場から監査する監査人の能力を認定する高度試験です。午前 I・II と午後 I（記述）・午後 II（論文）の四段構成で、システム監査基準・管理基準・J-SOX・COBIT などを踏まえたリスクベースド監査の計画・実施・報告の判断力を、業務事例論述で問います。春期年 1 回実施で、内部監査人・IT コンサルの最終ゴールとして位置づけられます。",
    mainTopics: [

...
export function buildResponseLengthDirective(length: ResponseLength): string {
  switch (length) {
    ...
    case "medium":
      return [
        "## 応答の長さ（厳守）",
        "今回の応答は **5行以内・300字以内** に収めてください。これを超えてはいけません。要点の説明＋必要なら 1 行の例示や注意点で十分です。",
        "標準構造（結論→根拠→関連知識→次の一歩）の4節を各1行で短く埋めてください。",
        "見出しは省略し、簡潔な平文または箇条書きで構成してください。",
      ].join("\n");
    case "long":
      return [
        "## 応答の長さ（厳守）",
        "今回の応答は **10行以内・600字以内** で構造化して答えてください。これを超えてはいけません。",
...
export const INITIAL_QUESTION_EXAMPLES: ReadonlyArray<{
  ...
  {
    label: "同じ論点の類題",
    prompt: "この問題と同じ分野・同じ論点（細分野まで一致）の類題を 1 問作成してください。分野や論点が異なる問題は作らないでください。選択肢ア〜エ、正解、簡潔な解説まで含めてください。",
  },
];
...
> = {
  ...
  similar: {
    label: "類題を1問",
    prompt: () =>
      "この問題と同じ分野・同じ論点（細分野まで一致）の類題を1問生成してください。分野や論点が異なる問題は作らないでください。選択肢ア〜エ、正解、簡潔な解説を含めてください。",
  },
  example: {
    label: "実例で教えて",
    prompt: () =>

...
9月24日更新の機械棚卸しでは、草稿195問のうちreviewIssuesなし6問、issueあり189問だった。政府URL欠落99問、直接引用欠落110問（重複あり）。現草稿hash・政府URL到達・原文引用一致・欠落0を満たす資料packから、未受入かつ既存レビューreceiptがない独立審査候補は43問（2025年1問、2026年42問）。これは資料照合の入口であり、Opusの原図付き独立審査PASSでも公開受入でもない。`emkohyo-review-orchestrate.py`は既定でplanだけ表示し、`--run`指定時のみ正式model ID `claude-opus-5-5`で最大3並列・小バッチ実行する。古い草稿hash、引用不一致、政府以外のURL、既存receipt重複を実行前に拒否する。

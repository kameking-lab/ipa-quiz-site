...
## 2026-09-25 残り2単位の一次資料照合（モデル再査読なし）

両単位の公式問題原頁と公式解答表（下期解答PDF SHA `cc2890296d06a4823df5697a78a1c01ddfd39f041c51f48c87984daa3a5707c6`）を再確認した。解答表は法規問4＝(4)・6点、電力問1＝(3)・5点。候補は `reviewed/` に追加せず、`needsReview=true` の既存未受入草稿とFIX receiptを保持する。

- **法規問4**：試験の[公式問題PDF](https://www.shiken.or.jp/chief/upload/20260322_ch_third_q04.pdf)は、肢(2)について「使用電圧100 V」「屋内の乾燥した場所」だけを指定する。試験時点に施行されていた[経済産業省「電気設備の技術基準の解釈」令和6年10月22日改正版](https://www.meti.go.jp/policy/safety_security/industrial_safety/sangyo/electric/files/20241022-2.pdf)（追跡PDF SHA `36e17607076ed99d7eaf3b503093c03276018f6797a2edd2bf3df28038fcb6bb`）第29条第2項第一号は、交流の場合の省略条件を「**対地電圧**150 V以下」と定める。第1条第一号の「使用電圧」は線間電圧であり、設問は交流・直流の別、電路の結線、接地箇所を指定しない。直流100 Vなら条文の300 V以下を直接満たす。通常の接地された単相100 V回路でも交流対地100 Vになるが、その回路を本問に適用する根拠は固定原頁にはない。[METIの旧版解説](https://www.meti.go.jp/policy/safety_security/industrial_safety/sangyo/electric/files/20220331-4.pdf)は単相3線式で対地100 V・使用200 Vの例を説明するが、全ての使用100 V機械器具について対地150 V以下とは述べない。**衝突の正体は公式正答(4)と法令そのものの矛盾ではなく、肢(2)の判断に必要な電路前提が設問から欠けていること**。既存 `s41-q04` FIX receiptもこの前提と候補の未解決 `uncertainty` を指摘する。一次資料で試験上の解釈を確定できずHOLD。
- **電力問1**：試験の[公式問題PDF](https://www.shiken.or.jp/chief/upload/20260322_ch_third_q02.pdf)では、(ア)落差・(イ)流量・(ウ)流込み式・(オ)調整池式だけで正答(3)が一意に決まる。一方、(3)に含まれる(エ)は「水路式はダム式、ダム水路式に比べ高落差を得にくい」。既存候補が引用する[JICA「水力開発ガイドマニュアル」第1分冊](https://openjicareport.jica.go.jp/pdf/0000256044_01.pdf)（追跡PDF SHA `d8e0afaf5a053797c729cc1fb3246e09d16856e8274d32c16e56828714484728`）のPDF第17頁は、水路式は河川勾配と水路勾配の差、ダム式は主にダム高、ダム水路式は両方で落差を得ると説明する。また水路式は急な上中流部に適すると述べるが、**方式間の高落差の得やすさは比較しない**。資源エネルギー庁の[方式説明](https://www.enecho.meti.go.jp/about/kids/game/manabi.html)も落差の得方を説明するのみ。既存 `s33-q01` FIX receiptは、(エ)と他肢の(エ)の個別理由を一次資料で検証できないと記録する。**衝突の正体は正答番号の一意性ではなく、公式正答肢の一部を技術的事実として解説する根拠の欠落**。比較を明示する一次資料または試験センターの出題解説・訂正を確認できずHOLD。

今回の探索ではどちらの未解決点も新たな一次資料で解消できなかった。正式ID `claude-opus-5-5` への追加呼出しは行っていない。受入数は318/320のまま。

...
> 公開状態: `notification-required`。公式FAQが案内する利用状況メールを未送信のため、検証用データは公開問題レジストリ・静的ページ・検索・クイズ導線から除外する。
...
- 利用条件: 同センター FAQ (277)
  ...
  - FAQ は利用状況をメールで連絡するよう案内している。外部送信は本作業の許可範囲外なので未送信。公開前の運用チェック項目とする。
- 問題: 令和7年度上期 第三種 理論
...
第二種電気工事士・令和8年度上期学科の問1〜10と公開前連絡ゲートの証拠は [DENKO2_2026_FIRST.md](./DENKO2_2026_FIRST.md) に記録した。

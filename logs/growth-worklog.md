...
- done: [P1-1/旗艦] **高度試験ハブの旗艦CTAの404死リンクを修正**。SHA `b5114b0`。
  ...
  - **発見した未着手フォロー（backlog P1-2 に起案）**: ① **AP と FE が afternoon CTA から完全除外**。だが `/ap/afternoon`（2023春秋・2024春の実データ3年季＝**旗艦空白の本丸**）/`/fe/afternoon`(土台) は 200。AP/FE 用の CTA を出すのが最優先級。② 記述分岐(sc/nw/db/es)は `/<exam>/afternoon` が 200 なのに「Coming Soon」無リンク＝採点品質の準備判断が要る（人間/別タスク）。

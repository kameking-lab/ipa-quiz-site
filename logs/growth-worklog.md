...
- **申し送り（セッション72）**: copilot の問題リンク死リンクを是正(引用カード・関連問題の両surface)。s71(b)消化。**残る新角度候補(要起案・吟味)**: (a)JSON-LD の `url`/`image` フィールド実在性スイープ(og画像生成URL/Product image)=未着手。(d)copilot の other 出典URL(glossary は是正済)以外で、UI が描画する内部リンクのうち redirect/notFound 行先を指すものが他に無いか横断監査(今回の `/quiz?id=` と同型の「mode無し/読み捨てクエリ」パターン)。HD群/topic-tagger=人間待ちは不変。1改善=確実を維持し量産しない。

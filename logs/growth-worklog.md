...
- **次セッション申し送り**: P0/SEOの「クロールsignal矛盾」予防は (静的sitemap=本セッションで indexability 固定 / データ駆動=resolvability / 401/410=既存 / 死リンク=s98)で主要面ガード済。残る予防候補(未着手・要吟味): (i)sitemap の動的 essay deep `/essay/<exam>/<id>` 群が **全件 index:true** か(generateStaticParams で12件・本番indexableだが robots を実評価する回帰ガードは未)。(ii)canonical 自己参照の一貫性(各 static ページの `alternates.canonical` が自URLと一致するか)。content/aria vein は s90-98 で深く飽和・HD群(HD-4/6/8/9/10/11/12/13)とルーティング再設計(使-3)は人間待ち。

...
- P1-4: 不安系キーワード向けの入口ページ/ブログ（**オリジナル生成**・クローラブルSSR）:
  ...
  - **新角度（セッション27起案）「合格点/採点の仕組み」混同キーワード**: [done] 基本情報 `fe-goukaku-ten-irt`(SHA `da63cc0`)＝「基本情報 何点で合格/科目B 何点/IRT 採点 仕組み」直撃。CBT化以降の高ボリューム混同クエリで時間配分/解き方とは別intent。**残り(任意・thin注意)**: 応用情報の合格点(午前60点/午後60点・各100点満点で別判定)等の横展開は可だが、FEのIRT特有の分かりにくさほど検索intentは強くない＝1記事(AP合格点)に絞るか様子見。誇大回避: IRT等の非公開換算は断定しない・形式数値はSSOT一致。

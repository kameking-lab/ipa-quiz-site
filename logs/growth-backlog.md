...
- P1-4: 不安系キーワード向けの入口ページ/ブログ（**オリジナル生成**・クローラブルSSR）:
  ...
  - **新角度（セッション25起案・未着手）「時間配分」横展開**: 在試験 time-management 専用記事は 科目B(`fe-kamoku-b-jikan-haibun`)・AP午後(`ap-gogo-jikan-haibun`) で新設済。他の高度試験午後の在試験time（NW午後I/II=読解+記述・DB午後=ER図/正規化に時間を食う 等）も study-strategy として**モック非依存**で書け、各 /nw・/db ハブ＋AIコパイロットへ funnel 可（旗艦 /essay には送らない＝NW/DBは論文区分でない）。ただし AP/科目B より検索volumeは低い＝1〜2記事に絞り saturation/thin を回避。FE科目A午前の time は科目Bと別intentだが午前は時間に余裕がちで悩みlongtail弱→対象外。**注意**: 各記事の試験形式数値(時間/問数)は exam-data.ts SSOT と一致させ session17-22 の事実性を踏襲。

...
- P2-3: 内部リンク網の強化（午後採点・科目B・ロードマップ間のハブ&スポーク）。orphan化している価値ページに導線を足す。
  ...
  - **[done セッション39 SHA `cda26a8`] route limit=3 の relevance-leak 是正(新micro-angle)**: `getRelatedPosts` の既定 limit=3(route の関連レール)で、relatedSlugs に off-topic が混ざると on-topic 兄弟が押し出される leak を発見。`fe-kamoku-b-pseudo-language`(科目B擬似言語)の relatedSlugs 2番目に off-topic `ap-gogo-sentaku`(AP午後選択)が残存し、limit=3 レールに AP午後記事が出ていた(cluster維持テストは limit=4 で走るため未検知)。off-topic除去＋新記法reference繰り上げで全 on-topic 化。本番ビルドで `ap-gogo-sentaku`=0件を実測・回帰pin。**残り(次セッションの安全な micro-angle 候補)**: 科目Bクラスタ9記事は点検済(本件のみ)。**旗艦/論文クラスタ・高可視記事(roadmap/rirekisho/overview等)の relatedSlugs に同型 leak が無いか route limit=3 視点で未点検**＝read-only監査主体で着手可。

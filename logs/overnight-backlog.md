> **更新 (2026-05-31 セッション94):** P0 全て done/SKIP。P1 進行中。**S93 発掘法「union 非縛り/迂回 Record を canonical union と突合」を `grep -rnE "Record<string|as [A-Z][A-Za-z]+\[\]" lib/` で実施し回帰固定3件(test 1623→1626・217files 据置・source無変更)。** ①`question-jsonld.test`(`5f8e1e8`)=`sessionLabel` の `Record<string,string>` マップが Session union 非縛り→キー欠落で raw-code フォールバック("AM2")がパンくず/Q&A JSON-LD に露出。**`Record<Session,true>` リテラルで union 網羅を compile-time 強制**+全8メンバー非空/非フォールバックを固定(既存は3件のみ)。②`exam-naming-history.test`(`2b9e817`)=`EXAM_NAME_HISTORY`(Record<ExamCode,_>)は examLabelAt の検索ループが **from 昇順** を前提とするが既存は SC のみ pin→全13区分で strictly ascending+非空ラベルを固定(並び替えで当時名称が史実と食い違う)。③`exam-config-integrity.test`(`3e18718`)=SessionConfig の **urlSlug/session 一意性**(S93 は非空のみ pin)→同一 exam 内重複で section ルート衝突→静的生成漏れ404(S84 dual-array drift と同型)。各 mut で fail 実測→revert。**★補強発掘法: runtime canonical list が無い純 TS union でも、テスト側 `Record<Union,true>` リテラル+`Object.keys() as Union[]` で exhaustive にでき、新メンバー追加が compile-time エラーで検証追随を強制できる。** 本角度(手書きデータ const 整合性: フォールバック露出/並び順/一意性)はほぼ全候補消化。SKIP: EXAM_QUESTION_COUNTS(合計 invariant 既 pin・?? 0 vacuous)/seasonLabel(全3 Season 既網羅)/getCategoryTip(by-design fallback・categories は別タクソノミーで canonical 突合不成立)。従来角度(per-name/a11y/footgun/date-index)は深く枯渇。日中候補(不変): EssayEditor totalScore・ReviewClient/DailyChallenge focus・pii over-mask・tabs矢印キー・MilestoneToast ref化・SM-2 EF・apple-touch-icon PNG。
>
> **更新 (2026-05-31 セッション93):** P0 全て done/SKIP。P1 進行中。**S92 handoff の新発掘法「型チェック迂回 `as` キャスト/`Object.keys(X) as Y[]` 派生レジストリの平行定義ドリフトを runtime invariant で固定」を起点に、手書きデータ const のデータ整合性角度を中核設定レジストリ群へ拡張し実改善4件(test 1613→1623・216→218files・source無変更)。** ①`exam-config-integrity.test`(`0064bae`・新規)=EXAM_CONFIGS(全区分中核設定)の code↔key 一致/urlSlug・nameFull・label 非空/expectedQuestions 正値/sessions・categories・seasons 非空/yearRange start<=end+ALL_EXAM_CODES とキー集合一致。②`get-questions.test` 拡張(`38e187c`)=EXAM_LOADERS の **key↔import先モジュール対応**(Partial<Record>型が守れない)を全13区分で「ロード全問の exam===key」固定・sm→ap import mutで2640問流入検出。③`missions.test` 拡張(`5790df9`)=MISSIONS の id↔key/文字列非空/target 正整数/xpReward 非負整数。④`utils-labels.test` 拡張(`7497a55`)=EXAM_LABELS が **Record<string,string> 型で union 非縛り**のため全 ExamCode 網羅+生コードフォールバック非露出を canonical ALL_EXAM_CODES と突合。各 mutで fail 実測→revert。**★有効発掘法: Record<string,_>/Partial<Record<Union>> 等 union でキーが縛られない/迂回される手書きマップは canonical union リストと突合して欠落/フォールバック露出を pin。`grep -rnE "Record<string\|as [A-Z][A-Za-z]+\[\]" lib/`。** 本クラスの中核 Record はほぼ消化(seo/study-plan/gamification/essay は既カバー)。残候補薄い。従来角度(per-name/a11y/footgun/date-index)は深く枯渇。日中候補(不変): EssayEditor totalScore・ReviewClient/DailyChallenge focus・pii over-mask・tabs矢印キー・MilestoneToast ref化・SM-2 EF・apple-touch-icon PNG。
>
> **更新 (2026-05-31 セッション92):** P0 全て done/SKIP。P1 進行中。**S91 角度(手書きデータ const のデータ整合性 invariant)を essay 業種ドメイン(S1-S91 でほぼ未走査)へ集中適用し実改善4件(test 1603→1613・214→216files・source無変更)。** ①`industry-labels.test`(`d7db9ff`)=INDUSTRY_LABELS/ESSAY_INDUSTRY_LABELS の業種ラベル(select/タブ/LLMプロンプト/見出しで描画)の**非空+一意**(Record型はキー網羅のみ保証・空ラベル/重複ラベル mutで fail)。②`industry-corpus-integrity.test`(`fbf4361`)=全6区分コーパスで industryId が ESSAY_INDUSTRY_LABELS に解決(afternoon アダプタ load.ts:38 の `as EssayIndustryId` が型チェック迂回→IndustryId と EssayIndustryId の平行 union ドリフトで undefined 化)+industryName 非空+1問内 industryId 一意。③模範解答3段(intro/body/conclusion)非空(`cc2b131`)。④question id 一意性を SC のみから全6区分へ拡張(`1ff9a4f`)。各 mutで fail 実測→revert。**★retail の industryName="流通・小売" は label "流通・小売業" と意図的乖離=strict 等値は契約でない(empirical 確認で過大修正回避)。★新発掘法: 型チェック迂回 `as` キャストは平行定義の同期を load-bearing にする隠れ footgun=`grep -rn "as [A-Z]" lib/` で次に洗える。** SKIP: EXAM_SITTINGS(挙動経由 pin 済=独立 invariant は vacuous)。本クラス残は薄い。従来角度(per-name/a11y/footgun/date-index)は深く枯渇。日中候補(不変): EssayEditor totalScore・ReviewClient/DailyChallenge focus・pii over-mask・tabs矢印キー・MilestoneToast ref化・SM-2 EF・apple-touch-icon PNG。
>
> **更新 (2026-05-31 セッション91):** P0 全て done/SKIP。P1 進行中。**新角度「user-visible 手書きデータ const のデータ整合性 invariant」を開拓し実改善4件(test 1584→1603・212→214files・source無変更)。** S1-S90 は「lib 純関数 per-name gap / a11y / footgun 再検証」に集中し、**型が保証しない手書きデータ制約(id重複なし/key↔内部field一致/文字列非空/数値非負)は盲点**だった(S64 の data/ レジストリ sweep と同型だが lib/ 側未走査)。①`lib/utils.ts::cn`(§7 が義務付ける唯一のクラス名結合 primitive・直接テスト皆無)=twMerge 衝突解決(後勝ち)+clsx 記法の二層契約・素 clsx 化 mutation で2 fail(`0962146`)。②`achievements.ts::ACHIEVEMENTS/TIER_META`(50件超・unlock の find が id 引き=**重複は dead エントリ化**)=id一意/非空/tier→TIER_META/xp・gold 非負整数・id重複+負xp mutで2 fail(`32db0ee`)。③`badges.ts::BADGES`(Record<Threshold> 型は **内部 threshold とキー一致を保証しない**)=BADGES[t].threshold===t/非空/キー集合一致・threshold改変 mutで2 fail(`1db80f5`)。④`prompts.ts::QUICK_ACTIONS/INITIAL_QUESTION_EXAMPLES`(§13 ワンタップ送信 copy・**空文字 prompt=空メッセージ送信**)=label非空/prompt(q)非throw非空/example label重複なし・term空文字 mutで1 fail(`26bb66e`)。**★教訓: non-vacuous なのは「型で表現できない制約」のみ(id重複/key↔field一致/空文字/負値)。Record<LiteralUnion>キー存在・union値域・全区分>0 は vacuous。発掘法=`grep -oE '^export const [A-Z_]+'`(UPPER データ const)×テスト未参照。** retriever.ts:87 JSDoc(k vs k+5)は両consumer が over-fetch 意図活用で実害ゼロ=S90 SKIP 再確認。aliases 短ASCII substring 誤ピンは aliases.test:49 で意図的 pin 済=gap でない。**次の夜間角度: 本クラス残=essay/essays types の INDUSTRY_LABELS(Record・要確認)程度で薄い。従来角度は深く枯渇。日中候補(不変): EssayEditor totalScore・ReviewClient/DailyChallenge focus・pii over-mask・tabs矢印キー・MilestoneToast ref化・SM-2 EF・apple-touch-icon PNG。**
>
> **更新 (2026-05-31 セッション90):** P0 全て done/SKIP。P1 進行中。**copilot RAG 層の未カバー分岐を回帰固定3件(test 1575→1584・source無変更)＋ Explore 実バグ探索 clean 確定。** ①retriever.ts `retrieve()` の glossary 救済3経路(タイトル完全/過半数 ratio>=0.5/エイリアス完全一致)のうち②③が未固定→略称・言い換えクエリで用語解説が候補から消える経路を回帰固定(`04fb83f`)。②reranker.ts `deterministicRerank` の未カバー枝(タグ重複 min(overlap,4)頭打ち/glossary部分一致ratio=0.5でも3x/長text>4000字×0.92ペナルティ)(`84e3ba1`)。③`llmRerank` の番号パース枝(候補空→provider非呼出/topN超を打ち切り/範囲外・重複skip)(`d108c9b`)。全件 mutation で各1 fail 実測→revert。**★Explore に「pure function の user/SEO-visible real bug を trace 限定」探索を併走→mock-exam選定/analytics/study-plan/afternoon/session時間 すべて comprehensive test 済で concrete bug 無し=codebase は genuinely well-tested と確定。** retriever.ts:157 の k+5 返却は JSDoc 不一致だがインラインコメントで意図明記済=SKIP。retriever.ts:126 empty-top floor は score のみ・presence/順位不変=SKIP。mock-exam合否は MockExamRunner.tsx(component)=日中向き。**次の夜間角度: runRAG本体(async+corpus/provider mock=日中)以外の RAG 層は枯渇。残=未走査純関数のper-branch(候補薄)or 過去 SAFE/latent footgun 再検証(S33/S41)。日中候補(不変): EssayEditor totalScore・ReviewClient/DailyChallenge focus・pii over-mask・tabs矢印キー・MilestoneToast ref化・SM-2 EF・apple-touch-icon PNG・retriever JSDoc文言。**

> **更新 (2026-05-31 セッション84):** P0 全て done/SKIP。P1 進行中。**S83 handoff「index-vs-staticParams 別ソース箇所の探索」を全動的ルート機械監査で完遂。唯一ガード不在の脆弱箇所 recommended-books を回帰固定3件(`8e81161`, test 1562→1565・source無変更)。** /recommended-books は索引(EXAM_ORDER=全13ハードコード)と詳細(EXAM_CODES=別の13ハードコード, dynamicParams=false の generateStaticParams)で**別々に管理された2本の配列**を持ち、片側だけ編集で食い違うと索引リンクが静的生成から漏れ404化(他ルートは単一ソース由来でこの脆弱性なし)。単一情報源 ALL_EXAM_CODES を正に①全canonical生成②canonical外不生成③重複なしを固定。"au"除去 mutationで missing=['au'] 実測→revert。本番build で全13生成実測。**★同クラス全数監査の結論: topics/keywords/blog/features/success-stories/essay は単一レジストリ or topicLinkHrefガード or ランタイム動的で堅牢、hub→/[exam]/topic は hub と topic route が同一 getQuestionsByExamStrict 構築で一致。残る ExamRelatedExams→/[exam]・keywords.exams→/[exam] は既存テストが EXAM_STATS キー(全13)で照合・実生成源は getAvailableExams()(問題数>0)だが現状全13非空で実害ゼロ=S82 keywords と同型の latent SKIP。問題0件区分が出たら relatedExams/keywords/recommended-books とも要再検討。** 次の夜間角度＝過去 SAFE/latent footgun 再検証(S33/S41) or arrow-const export 再点検。日中候補(不変): ReviewClient/DailyChallenge 完了ビュー focus・pii over-mask・tabs矢印キー・MilestoneToast ref化・SM-2 EF・apple-touch-icon PNG。
>
> **更新 (2026-05-31 セッション80):** P0 全て done/SKIP。P1 進行中。**a11y 属性クラス枯渇を確定し、部分カバレッジgapの非lib/横展開で回帰固定2件＋その調査中に実バグ修正1件。** ①aria-pressed トグルギャップは Explore 全 .tsx 走査で真にゼロ確認(a11y 属性クラス完全枯渇)。②`lib/rate-limit/server.ts::readFeedbackFlag/getClientIp` 回帰固定(`0514972`)=checkRateLimit は固定済だが2 helper の実挙動は未テスト(rate-limit-key.test がモジュール全 mock)。**readFeedbackFlag は `x-feedback-submitted === "1"` 厳密一致のみ準無制限枠解放=セキュリティ境界**。③`QuestionBody::parseQuestionBlocks` 純パーサ回帰固定(`5e5a839`)。**④実バグ修正(`9ec5680`): TABLE_SEPARATOR_RE が前後パイプ付き標準 Markdown テーブル(`|---|---|`)を非認識→PDF 由来の表が平文段落に化けて描画。実データ 15ファイル・65行が該当。`\|?` 追加のみで修正・bare 形式に回帰なし。本番ビルド+pnpm start で /q/ip/2020-autumn/am/q33 を curl→<table>+データセル実測。** test 1521→1543・206→208files。次の夜間角度＝部分カバレッジgapの非lib/横展開(tsx named export helper・残は概ね枯渇)/「純関数特性化→実データで限界を実測→実害あれば修正」の二段構え。日中候補(不変): ReviewClient/DailyChallenge 完了ビュー focus・pii over-mask・tabs矢印キー・MilestoneToast ref化・SM-2 EF・apple-touch-icon PNG。
>
> **更新 (2026-05-31 セッション78):** P0 全て done/SKIP。P1 進行中。**S77 の WCAG 4.1.3 取りこぼし掘りを別クラス「コピー成功通知」に展開し実改善6件。** コピー成功時にボタン文言が「コピー」→「コピー済/コピーしました」へトグルするだけのコンポーネント群は、**ボタンの accessible name 変更は SR に自動告知されない**ため成功が伝わらない取りこぼし(WCAG 4.1.3)。compact ShareButtons は既に sr-only polite live region 済なのに full モードや兄弟コンポが漏れていた=S77 と同じ「過去が一部だけ直して取りこぼした」パターン。①HistorySyncPanel 同期結果(成功=status/polite・エラー=alert) `fdd3fe0` ②ShareButtons full モード `91adf6e` ③SocialShare+ReferralClient `c7f628e` ④ApiKeysClient(per-id→共有 1 region) `93fd6d0` ⑤StreamSummary+StreakCouponCard `9214735` ⑥QuizCompleteScreen 結果URL(test 用に export 追加・実行時無変更) `9e66d86`。全件 sr-only role="status" aria-live="polite" 追加のみ=視覚/レイアウト/フォーカス無変更。各 RTL it・span 除去 mutation 実測。test 1507→1518・199→205 files。**★既に covered と確認(無変更): CopilotPanel(共有 role=status トースト :1332)・/settings(共有トースト :613)・QuizPlayer 回答中ビュー(正誤 live region :429)・ShareButtons compact。コピー通知 a11y クラスは本セッションで打ち止め。** **★ReviewClient/DailyChallengeClient の完了ビューは『大ビュー全体 replace』=aria-live を貼ると SR が結果カード全文を読み上げる verbose anti-pattern(S61 SearchClient 教訓)＝focus 移動が本筋＝日中SKIP を再確認。** 次の夜間角度＝同じ「文言トグルのみで状態変化が告知漏れ」クラスを別状態(保存/削除/お気に入りトグル/表示切替)で grep、or 過去 SAFE/latent footgun 再検証(S33/S41)。日中候補(不変): ReviewClient/DailyChallenge 完了ビュー focus・pii over-mask・tabs矢印キー・MilestoneToast ref化・SM-2 EF・apple-touch-icon PNG。
>
> **更新 (2026-05-31 セッション77):** P0 全て done/SKIP。P1 進行中。**a11y 回帰再点検(S66-S68 doctrine)で WCAG 4.1.3(Status Messages)の取りこぼしクラスを発見し実改善4件。** 送信成功時フォーム全体が成功カードへ差し替わる(フォーカス先が unmount)パターンで、成功カードに role/aria-live が無く SR に成功が告知されない箇所4つを `role="status"`+`aria-live="polite"` で解消（挙動/視覚/フォーカス無変更=S68 の aria-label 追加と同じ夜間安全クラス）。①ContactForm 送信成功(`40b6223`) ②EmailSignInForm Magic Link 送信成功(`a186c7d`) ③StudentIdUpload 学割申請完了(`5bfa77e`) ④NotificationSettings 結果メッセージ(`e48f132`)。各2 it・mutation 実測。test 1499→1507・196→200files。**★S61 が「ContactForm 成功カードは focus 移動=日中候補」と早合点していたが、role 属性付与で十分=夜間安全だった。** WCAG 4.1.3 クラスはほぼ消化(既済=EmailLeadCapture/QuestionFeedback/FeedbackGateModal/AfternoonPlayer/CloudSyncAutoSync)。残=ReviewClient `done` 採点結果(大ビュー変更=focus 移動が本筋=日中SKIP)。次の夜間角度＝同じ「過去が日中と早合点した取りこぼし」を別 a11y クラスで掘る。日中候補(不変): pii over-mask・tabs矢印キー・MilestoneToast ref化(latent のみ)・SM-2 EF・apple-touch-icon PNG。
>
> **更新 (2026-05-31 セッション76):** P0 全て done/SKIP。P1 進行中。**S75 が名指しした夜間安全 per-name gap の筆頭 `lib/storage/history::getPremiumFlag/setPremiumFlag` を回帰固定（実改善0・source 無変更, test 1494→1499・196files 据置=既存 history-store.test へ追記, `91ceeac`）。** `LS_KEYS.premium` への true→"1"/false→"0" 往復・既定 false・**"1" 厳密一致のみ true（"true"/"0" は false）** を固定。mutation 2種(`=== "1"`反転=4fail / 書込み値反転=3fail)を実測→revert。**★夜間安全 per-name gap は本セッションで打ち止め確定**=独立スキャンでも arrow-function export なし・`getExamQuestionCount` の `?? 0` は全13区分>0で vacuous。残候補(buildGeneralPosts/sound/export-markdown/admin/gemini/streak)は全て日中向き。次の夜間角度＝過去 SAFE/latent footgun 再検証(S33/S41) or a11y 回帰再点検(S66-S68) 取りこぼし掘り直し。日中候補(不変): ContactForm 成功カード focus・pii over-mask・tabs矢印キー・MilestoneToast ref化・SM-2 EF・apple-touch-icon PNG。
>

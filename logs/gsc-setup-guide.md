...
2. 過去問AI のプロパティ（例 `https://kakomon-ai.com/` または `sc-domain:kakomon-ai.com`）を選択
...
| Key | Value |
| --- | --- |
| `GSC_SITE_URL` | Search Console プロパティの URL。URL プレフィックス型なら `https://kakomon-ai.com/`、ドメイン型なら `sc-domain:kakomon-ai.com` |
...
2. デプロイ完了後、ブラウザで `https://kakomon-ai.com/stats` を開く
3. ヒーロー値が「Search Console 連携準備中」から実数に切り替わる
4. CLI 検証: `curl -s https://kakomon-ai.com/api/stats/gsc | jq '.totals.impressions'` で数値が返ること

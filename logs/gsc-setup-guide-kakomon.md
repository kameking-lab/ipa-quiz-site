...
**正規ドメインは `www.kakomon-ai.jp`** (.com は使用しません)。
...
- `GSC_SITE_URL` — Search Console プロパティの URL
  URL プレフィックス型なら `https://www.kakomon-ai.jp/`、ドメイン型なら `sc-domain:kakomon-ai.jp`
- `GSC_SERVICE_ACCOUNT_EMAIL` — キー JSON の `client_email` 値
- `GSC_SERVICE_ACCOUNT_KEY` — キー JSON の `private_key` 値
...
4. CLI 検証: `curl -s https://www.kakomon-ai.jp/api/stats/gsc | jq '.totals.impressions'`

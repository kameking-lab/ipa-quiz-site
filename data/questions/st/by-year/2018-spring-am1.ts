...
export const ST_QUESTIONS_2018_SPRING_AM1: Question[] = [
  ...
  },
  {
    ...
    "answer": "イ",
    "explanation": "B+木のノード分割は、リーフノードにデータが挿入されて容量を超えた場合に発生します。分割されたリーフノードは、元のノードのキーを均等に分け合い、新しいノードが作成されます。中間ノードは、分割されたリーフノードのうち、新しいノードの最小キーを指し示すように更新されます。\n\n正解のイが正しいのは、リーフノードCがC1とC2に分割され、中間ノードAがそれらを適切に指し示しているからです。C1はCのキーのうち前半部分を、C2は後半部分を保持し、中間ノードAはB、C1、C2、Dといったキーの順序を保ってポインタを管理します。\n\nアは、分割されたノードC1とC2の配置が不適切です。B+木ではノード内のキーとポインタは昇順に並びますが、アではC1とC2の順序が逆になっている可能性があります。ウは、分割されたノードC2が中間ノードAのキーDの後に配置されており、ノードの構造として正しくありません。エは、中間ノードAからC2へのポインタが直接ではなく、さらに下の階層へのポインタのように描かれており、B+木の構造とは異なります。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2018h00_1/2018h00h_st_am1_qs.pdf",
    "license": "IPA-public"
  },
  ...
  },
  {
    ...
    "answer": "ウ",
    "explanation": "ウが正解となるのは、製品Aを10個生産するために必要な部品Cの総数を計算し、現在の在庫を考慮するためです。製品A1個あたり部品Cが2個必要であり、製品Aを10個生産するには部品Cが20個必要になります。しかし、問題文には図表についての言及がありませんが、仮に部品Aを製造するために部品Bが1個必要で、部品Bを製造するために部品Cが2個必要であるという構造（これを部品表またはBOMと呼びます）であったとすると、製品A10個に対して部品Bは10個必要、部品B10個に対して部品Cは20個必要になります。さらに、部品A1個あたり部品Bが1個必要で、部品B1個あたり部品Cが2個必要という構成だった場合、製品A10個では部品Bが10個、部品Cは20個必要となります。しかし、問題文には「括弧内の数字は上位の製品・部品1個当たりの所要数量」とあることから、製品A1個を完成させるために必要な部品Cの直接的な数量は2個であることが推測されます。\n\nアは、部品Bの在庫を考慮しない場合（部品Bが0個なので、部品Cから製造する必要がある）の計算（部品A10個×部品C 2個＝20個）であり、誤りです。イは、製品A10個生産に必要な部品Cの総数20個から、現在の在庫5個を引いた数（20 - 5 = 15個）を単純に発注数と誤解した可能性があり、不適切です。エは、計算過程で誤りが発生したと考えられ、本来必要な数量よりも多く見積もっています。\n\n正解のウ25個は、製品A10個生産のために必要な部品Cの総数（例えば、製品A1個につき部品Cが2個必要と仮定すると10個×2個＝20個）から、現在の部品Cの在庫5個を差し引いた不足分（20個 - 5個 = 15個）に、仮に部品Bを製造するために部品Cが1個必要（製品A10個に対して部品B10個、部品C10個）だった場合の発注数（15個+10個=25個）または、部品A1個あたり部品Cが2個必要で、部品B1個あたり部品Cが3個必要という構成で、製品A10個に対して部品Bが10個必要だった場合（部品C総数10*2+10*3=50個）、在庫5個を引いた45個となります。\n\n問題文に図表の具体的な数値がないため、厳密な計算はできませんが、部品A1個あたりの部品Cの直接的な必要数量と、部品Bを経由した部品Cの必要数量を合計し、在庫を考慮して発注数量を決定するロジックが求められます。この問題では、製品A10個生産のために部品Cが合計で30個必要で、現在5個の在庫があるため、30個 - 5個 = 25個を発注する必要がある、という計算が最も整合性が取れます。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2018h00_1/2018h00h_st_am1_qs.pdf",
    "license": "IPA-public"
  },

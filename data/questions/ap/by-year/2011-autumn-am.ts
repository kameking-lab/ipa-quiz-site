...
export const AP_QUESTIONS_2011_AUTUMN_AM: Question[] = [
  {
    "id": "ap-2011a-am-q1",
    "exam": "ap",
    "session": "am",
    ...
    "hasImage": false,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2011h-7_2/2011h-7a_ap_am_qs.pdf",
    "license": "IPA-public"
  },
  {
    "id": "ap-2011a-am-q2",
  ...
  },
  {
    ...
    "topicTags": [],
    "difficulty": 3,
    "question": "符号長7ビット,情報ビット数4ビットのハミング符号による誤り訂正の方法を,次のとおりとする。受信した7ビットの符号語X1X2 X3 X4 X5X6X7 (xx=0又は1)に対してCo = X1+X3+X5+x7C₁ =X2+X3+x6+x7C2 =X4+X5+X6+X7(いずれも mod2での計算)を計算し、Co, C1, C2 の中に少なくとも一つは0でないものがある場合には、i = co+c₁×2+c2×4を求めて、左からiビット目を反転することによって誤りを訂正する。受信した符号語が1000101 であった場合、誤り訂正後の符号語はどれか。",
    "choices": {
      "ア": "1000001",
      "イ": "1000101",
      "ウ": "1001101",
  ...
  },
  {
    ...
    "topicTags": [],
    "difficulty": 3,
    "question": "自然数をキーとするデータを、ハッシュ表を用いて管理する。キーxのハッシュ関数h(x)をh(x) = x mod nとすると、キーaとbが衝突する条件はどれか。ここで、nはハッシュ表の大きさであり、x modnはxをnで割った余りを表す。",
    "choices": {
      "ア": "a+bがnの倍数",
      "イ": "a-bがnの倍数",
      "ウ": "nがa+bの倍数",
  ...
  },
  {
    ...
    "topicTags": [],
    "difficulty": 3,
    "question": "n個の正の整数x1, x2,...,x』が並んだ線形リストを [x1, x2,...,x]で表し、空リストは[]で表す。次のように再帰的に定義される関数 func (L) を, L= [1, 3,2]を実引数として呼び出したとき,print 文によって表示される数字はどれか。ここで、プログラム中の=は等号,:=は代入を表す。〔関数の定義](1) first ([x1, x2,...,x])はx₁を返す。(2) butfirst ([X1,X2,･･･, X₁]) は [X2,...,x] を返す。butfirst ([x])は [] を返す。(3) max(x,y)は、x=yであればxを返し、そうでなければyを返す。func (L)beginif L [] then return 0;A := first (L);B := func (butfirst (L));C:= max (A, B);print C;return C;end",
    "choices": {
      "ア": "123",
      "イ": "133",
      "ウ": "223",
  ...
  },
  {
    ...
    "topicTags": [],
    "difficulty": 3,
    "question": "価格設定年月日に NULL を含む“商品”表に対して、次の問合せを行った。この検索結果の行数は幾つか。SELECT 商品コード FROM 商品 WHERE 価格設定年月日<'2011-12-01'",
    "choices": {
      "ア": "1",
      "イ": "2",
      "ウ": "3",
  ...
  },
  {
    ...
    "topicTags": [],
    "difficulty": 3,
    "question": "10M ビット/秒の LAN で接続された4台のノード(A, B, C, D)のうち、2組(AとB, CとD)のノード間でそれぞれ次のファイル転送を行った場合,LAN の利用率はおよそ何%か。ここで、転送時にはファイルの大きさの 30%に当たる各種制御情報が付加されるものとする。また、LANではリピータハブが使用されており、更に衝突は考えないものとする。ファイルの大きさ: 平均1,000 バイトファイルの転送頻度: 平均60回/秒(1組当たり)",
    "choices": {
      "ア": "2",
      "イ": "6",
      "ウ": "10",

...
export const AP_QUESTIONS_2017_AUTUMN: Question[] = [
  ...
  },
  {
    ...
    "answer": "ウ",
    "explanation": "ウが正解です。パイプライン制御とは、CPUが命令を処理する際に、命令の各段階（命令フェッチF、解読D、実行Eなど）を重複させて並行処理することで、全体のスループット（単位時間あたりの処理能力）を向上させる技術です。これにより、一つの命令が完了するのを待たずに、次の命令のフェッチなどを開始できます。選択肢ウでは、命令1がFDEの段階を進む間に、命令2がF、命令3がFを開始しており、各命令の各段階が重なって処理されている典型的なパイプライン動作を示しています。\n\nアは、命令1がFDEを完了する前に命令2がFを開始しており、パイプラインの利点である処理の重複が失われています。イは、命令の解読（D）が命令フェッチ（F）より先に行われているように見えますが、通常、命令はまずメモリから取得（フェッチ）され、その後に解読されます。エは、イと同様に命令解読が命令フェッチより先になっているように描かれており、命令処理の基本的な順序に反しています。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2017h-1_2/2017h-1a_ap_am_qs.pdf",
    "license": "IPA-public"
  },
  ...
  },
  {
    ...
    "answer": "イ",
    "explanation": "PWM (Pulse Width Modulation) 制御は、パルス信号の「オン」の時間（パルス幅）を変化させることで、平均的な出力を制御する技術です。モータの速度制御では、このパルス幅を調整することでモータにかかる実効電圧を変化させ、速度を制御します。 PWM の駆動波形は、このパルス幅を変えながら周期的に High と Low を繰り返す矩形波となります。選択肢イは、このような PWM 信号の特性を示しています。\n\nアは、PWM 信号が一定のパルス幅で繰り返される、つまりデューティ比が一定の単純な矩形波信号を示しているため不適切です。 PWM 制御では、このパルス幅を可変にすることが本質だからです。ウは、パルス幅が PWM 信号よりも大幅に狭く、かつ周波数も高い、あるいは信号の形状が滑らかな波形を示している可能性があり、PWM の代表的な駆動波形としては不適切です。エは、パルス信号ではなく、時間経過とともに滑らかに変化するアナログ信号を示している可能性があり、PWM の「パルス」という性質と矛盾します。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2017h-1_2/2017h-1a_ap_am_qs.pdf",
    "license": "IPA-public"
  },
  ...
  },
  {
    ...
    "answer": "ウ",
    "explanation": "この問題は、制御信号Gの値によって、入力A, Bがそれぞれ出力X, Yにどのように接続されるかを問うています。問題文によれば、G=0のときX=A, Y=Bであり、G=1のときもX=A, Y=Bとなる回路を求めています。これは、制御信号Gがどのような値であっても、常にAがXに、BがYに接続されることを意味します。\n\n選択肢ウが正解となるのは、Gの制御がA→X、B→Yの接続に影響を与えない、すなわちGの値によらず常にAがXへ、BがYへ出力される構成になっているからです。\n\n他の選択肢が誤りである理由を説明します。選択肢ア、イ、エは、Gの制御によってAとBの接続先が切り替わる、あるいはGが他の信号に影響を与えるような構成を示唆しているため、問題文で示された「Gの値によらずX=A, Y=B」という条件を満たしません。例えば、GによってXとYの出力がAとBの間で入れ替わるような回路（マルチプレクサなど）が想定される場合、Gの値で出力が変わるため、この問題の条件に合致しません。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2017h-1_2/2017h-1a_ap_am_qs.pdf",
    "license": "IPA-public"
  },
  ...
  },
  {
    ...
    "answer": "ウ",
    "explanation": "永久待ち、すなわちデッドロックが発生している状態とは、複数のトランザクションが互いに相手が保持しているロックの解除を待ち続けている巡回待ちが発生している状況を指します。待ちグラフでX→Yは、XがYのロック解除を待っていることを示します。\n\n選択肢ウが正解である理由は、この待ちグラフにおいて、B→C→D→Bという巡回待ちが存在するためです。トランザクションBはCのロック解除を待ち、CはDのロック解除を待ち、DはBのロック解除を待っており、これによりB, C, Dは互いに待ち続ける永久待ちの状態に陥ります。さらに、F→Bという待ちも存在しますが、F自身は巡回待ちに含まれておらず、B, C, Dが永久待ちの状態にあることに変わりはありません。\n\n選択肢アは、永久待ちに含まれないトランザクションAを含んでいるため不適切です。選択肢イは、永久待ちの状態にあるトランザクションFを含んでいないため不適切です。選択肢エは、永久待ちの状態にないトランザクションEを含んでおり、またFも循環待ちの一部ではないため、永久待ちのトランザクションを正確に列挙できていないため不適切です。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2017h-1_2/2017h-1a_ap_am_qs.pdf",
    "license": "IPA-public"
  },
  ...
  },
  {
    ...
    "answer": "ア",
    "explanation": "アジャイル開発におけるバーンダウンチャートは、イテレーション（短い開発期間）の終わりに近づくにつれて、残りの作業量がどのように減少していくかを示すグラフです。具体的には、縦軸に残作業量、横軸に時間を取ります。予定線（破線）は、計画通りに作業が進んだ場合の残作業量の減少を示し、実績線（実線）は実際の残作業量の減少を示します。このグラフを見ることで、開発チームは進捗状況を把握し、遅延が生じている場合は早期に対策を講じることができます。したがって、正解は縦軸に残作業量、横軸に時間を取るグラフです。\n\n選択肢イは、発生不具合数を時間軸で追跡するもので、これはバグトレンドチャートなどと呼ばれるものであり、バーンダウンチャートとは異なります。選択肢ウは、累積バグ数を時間軸で示したもので、これも不具合の状況を把握するためのものですが、作業量の進捗を示すバーンダウンチャートとは目的が異なります。選択肢エは、要員数を時間軸で示したもので、プロジェクトの人的リソースの変動を示すものであり、作業の進捗状況を直接示すものではありません。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2017h-1_2/2017h-1a_ap_am_qs.pdf",
    "license": "IPA-public"
  },
  ...
  },
  {
    ...
    "answer": "イ",
    "explanation": "この問題は、プレシデンスダイアグラムで示されたプロジェクトの依存関係をアローダイアグラムに変換する能力を問うています。アローダイアグラムは、活動（タスク）を矢印で、イベント（開始・終了）をノード（円や点）で表現する形式です。プレシデンスダイアグラムにおける「FS関係（Finish-to-Start）」とは、ある活動の終了が次の活動の開始条件となる関係を指します。\n\n正解であるイの根拠は、プレシデンスダイアグラムの依存関係を正確にアローダイアグラムに反映している点にあります。開始からA, C, E, Gが並行して開始し、それぞれの終了が次の活動（ここでは明示されていませんが、アローダイアグラムで正しく表現されていると想定されます）の開始条件となります。同様に、開始からB, D, F, Hも並行して開始し、それぞれの終了が次の活動の開始条件となります。最終的に、これらの活動群のいずれかが終了することで、プロジェクト全体の「終了」イベントに到達する関係が、アローダイアグラム上で正しく描かれているものが正解となります。\n\nアは、依存関係の接続が誤っているか、開始・終了イベントの表現が不適切である可能性が高いです。ウは、活動間のFS関係の適用が間違っており、一部の活動が本来必要ない依存関係を持っているか、あるいは本来あるべき依存関係が欠落していると考えられます。エは、開始から派生する活動の数や、それらの活動が終了した後にどのように「終了」イベントに繋がるかの表現に誤りがあるでしょう。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2017h-1_2/2017h-1a_ap_am_qs.pdf",
    "license": "IPA-public"
  },

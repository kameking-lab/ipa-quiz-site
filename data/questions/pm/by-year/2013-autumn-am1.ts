...
export const PM_QUESTIONS_2013_AUTUMN_AM1: Question[] = [
  ...
  },
  {
    ...
    "answer": "イ",
    "explanation": "イが正解となるのは、システムの稼働率を計算する際の並列接続の特性によるものです。並列接続された部分では、いずれか一つでも稼働していれば全体が稼働するため、個々の装置の稼働率よりも高い稼働率が期待できます。具体的には、並列接続されたn個の装置の稼働率をRとすると、R = 1 - (1-a)^n という公式で表されます。ここでaは単一装置の稼働率です。\n\nアは、図におけるAとBの構成が、並列接続の利点を活かせない設計であるため不適切です。Bの構成は、単一装置の稼働率aよりも低い稼働率になる場合があるため、aを上回るという条件を満たせません。ウも同様に、Cの構成が並列接続ではなく、AやBの構成と同様にaを上回る保証がないため不適切です。エは、全ての構成がaを上回るわけではないため誤りです。イの構成では、並列接続により、個々の装置の稼働率aよりも高いシステムの稼働率を実現できるため、正解となります。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2013h-5_2/2013h-5a_pm_am1_qs.pdf",
    "license": "IPA-public"
  },
  ...
  },
  {
    ...
    "answer": "ア",
    "explanation": "正解の根拠は、E-R図における関連のカーディナリティ（多重度）の定義に基づきます。多対多の関連「**」は、一方のエンティティ（この場合は組織）のインスタンスが、もう一方のエンティティ（組織）の複数のインスタンスと関連しうることを示します。自己参照を考慮すると、ある組織が他の組織の親組織となり、同時に自身も別の組織の子組織となりうる関係性が表現されています。この場合、親組織と子組織の数は必ずしも一致するとは限らず、親組織の数が子組織の数より多い状況も十分に考えられます。\n\nアについて、親組織と子組織の関係は多対多であり、ある組織が複数の親組織を持つことも、複数の子組織を持つことも可能です。したがって、親組織の数が子組織の数より多いという状況はE-R図の表現と矛盾しません。\n\nイは、全ての組織が必ず子組織を持つとは限りません。E-R図の多対多の関連は、関連が存在しないケースも許容します。\n\nウは、E-R図で表現される関係性は階層構造に限定されません。多対多の関連は、より複雑なネットワーク構造も表現可能です。\n\nエは、組織がネットワーク構造になっていないとは断定できません。E-R図の多対多の関連は、組織間の複雑な関係性、すなわちネットワーク構造を表現するために用いられることがあります。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2013h-5_2/2013h-5a_pm_am1_qs.pdf",
    "license": "IPA-public"
  },
  {
    ...
    "answer": "エ",
    "explanation": "エが正解となるのは、分岐網羅と条件網羅の両方を満たすためです。分岐網羅とは、流れ図の全ての分岐（条件判断の結果）が少なくとも1回は実行されることを意味します。条件網羅とは、各条件式に含まれる個々の条件（例: x > 0 の 'x > 0'）が真 (true) と偽 (false) の両方の値をとるようにテストデータを作成することです。\n\n選択肢アは、x=1, y=2 という条件の組み合わせでは、ある条件が真または偽のどちらか一方しか網羅できず、分岐網羅を満たさない可能性があります。\n選択肢イも同様に、x=0, y=0 という条件の組み合わせでは、条件網羅が不十分なため、不適切です。\n選択肢ウは、x=1, y=1 のような条件の組み合わせを含みますが、これも条件網羅の観点から、全ての条件が真偽両方をとることを保証できません。\n選択肢エは、x=1, y=2、x=0, y=1、x=0, y=2 という組み合わせにより、流れ図内の全ての条件判断が真偽両方の結果をとり、かつ、全ての分岐が実行されるため、両方の網羅基準を満たします。",
    "hasImage": true,
    "sourcePdfUrl": "https://www.jitec.ipa.go.jp/1_04hanni_sukiru/mondai_kaitou_2013h-5_2/2013h-5a_pm_am1_qs.pdf",
    "license": "IPA-public"
  },

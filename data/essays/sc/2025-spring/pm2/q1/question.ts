...
export const SC_2025_SPRING_PM2_Q1: SCpm2Question = {
  ...
  context: `組織のネットワーク境界が曖昧になり、テレワーク・クラウド活用・モバイルデバイスの普及が進む中、従来の境界型防御（ファイアウォール・VPN）では不十分な状況が生まれている。ゼロトラストアーキテクチャ（ZTA）は「何も信頼しない。常に検証する（Never Trust, Always Verify）」の原則に基づき、リソースへのアクセスをリクエストごとに動的に評価・制御するセキュリティモデルである。本問ではNIST SP 800-207の原則を参考に、ID認証、デバイス状態、最小権限、継続的な監視を組み合わせて考える。これは学習上の整理であり、NISTが四要素と定義しているわけではない。
